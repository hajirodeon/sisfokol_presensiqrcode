			    ...
			    if ($i > 1) 
			    	{
					  ...
					  $i_qrcode = "$kdepan$i_kode";
						...
						if (!empty($tcc))
							{
							mysql_query("UPDATE m_item SET nama = '$i_nama' ".
											"WHERE kode = '$i_kode'");
							}
							...
							{
							//insert
							mysql_query("INSERT INTO m_item(kd, kode, nama, qrcode, postdate) VALUES ".
...
if ($_POST['btnSMP'])
	...
	if ((empty($e_nip)) OR (empty($e_nama)))
		...
		if ($s == "edit")
			{
			mysql_query("UPDATE m_item SET kode = '$e_nip', ".
							"nama = '$e_nama' ".
							"WHERE kd = '$kd'");

			//update
			mysql_query("UPDATE m_item SET filex1 = '$namabaru', ".
							"postdate = '$today' ".
							"WHERE kd = '$kd'");
	...
	if ($count != 0)
		...
		do 
			...
			if (empty($i_kode))
				...
				//update...
				mysql_query("UPDATE m_item SET kode = '$kodenya', ".
								"qrcode = '$kodenya' ".
								"WHERE kd = '$i_kd'");
				}
